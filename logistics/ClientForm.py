...
# XXX
# Fix or work around attibute quoting bug.
# Add .clear() method.
...
# Grabbed from 2.4 xml.sax.saxutils.  Modification: accept None.
...
def get_entitydefs():
    ...
    # unescape already does these three
    del entitydefs["&lt;"]
    del entitydefs["&gt;"]
    del entitydefs["&amp;"]
    return entitydefs

def unescape_attrs(attrs, entitydefs):
    escaped_attrs = {}
    for key, val in attrs.items():
        try:
            val.items
        except AttributeError:
            escaped_attrs[key] = unescape(val, entitydefs)
        else:
            # eg. "__select" -- yuck!
            escaped_attrs[key] = unescape_attrs(val, entitydefs)
...
def ParseFile(file, base_uri, select_default=False,
    ...
    for (name, action, method, enctype), attrs, controls in fp.forms:
        ...
        for type, name, attr in controls:
            attr = unescape_attrs(attr, entitydefs)
            name = unescape(name, entitydefs)
